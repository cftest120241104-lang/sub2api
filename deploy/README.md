...
- **Displays generated credentials** (ADMIN_PASSWORD, POSTGRES_PASSWORD, JWT_SECRET, etc.)
...
# Verify production env and compose
.\verify-production-env.ps1 -EnvFile .env
...
| `ADMIN_PASSWORD` | **Recommended** | *(auto-generated by docker-deploy.sh)* | Initial admin password |
...
> **Note:** The `docker-deploy.sh` script automatically generates `JWT_SECRET`, `TOTP_ENCRYPTION_KEY`, `POSTGRES_PASSWORD`, and `ADMIN_PASSWORD` for you.

For production, keep `POSTGRES_PASSWORD`, `ADMIN_PASSWORD`, `JWT_SECRET`, and `TOTP_ENCRYPTION_KEY` fixed in `.env`. Run `verify-production-env.ps1` before public deployment; it rejects weak example values and verifies that `VITE_GAME_SERVICE_BASE_URL` is an absolute URL.
